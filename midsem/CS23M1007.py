...
def DIC(transactions, M=100, min_support=0.5):
    ...
    # converting named unique items to numbers
    col2int = {}
    for v, k in enumerate(transactions.columns):
        col2int[k] = v

    # unique items in database
    unique_items = [
        set([e[0]]) for e in enumerate(transactions.columns)
    ]
    ...
    while len(dashed_box) != 0 or len(dashed_circle) != 0:
        for it in range(l, l + M):
            # all items which occur in transaction
            t = transactions.iloc[it]
            # convert unique item names to encoded number
            t = set([col2int[k] for k in t[t].index])
            # print(t)
            ...
            for c in dashed_box:
                ...
                if windows_counted[c] == len(transactions) // M:
...
freq_itemsets_dic = DIC(encoded[:18000], M=2000, min_support=0.1)
